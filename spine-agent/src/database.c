...
hostconfig ReadWWWConfiguration(char * hostid) {
	...
	char * query = mkString("SELECT www.ServerName, www.ServerAlias, www.DocumentRoot, www.htaccess, sysusers.login AS user, ",
							...
							"SEPARATOR ' ') ELSE 'NaN' END AS htusers FROM www JOIN sysusers ON sysusers.id = www.user_id JOIN ",
							...
							"(www_users.id = www_users_access.user_id AND www.htpasswd > 0) WHERE status = 'A' AND www.system_id = (SELECT id FROM ",
